...
export const ServiceTypesList = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("service_types").select(`
          *,
          sub_services:service_types!parent_service_id(*)
        `).is('parent_service_id', null).order('name');
  ...
  return <div className="space-y-6">
      ...
      <Tabs defaultValue="all" className="w-full">
        ...
        <TabsContent value="all" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {serviceTypes?.map(service => <ServiceTypeCard key={service.id} service={service} onEdit={() => {
        ...
        <TabsContent value="standalone" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {serviceTypes?.filter(s => s.service_type === 'standalone').map(service => <ServiceTypeCard key={service.id} service={service} onEdit={() => {
            setEditingService(service);
            setIsDialogOpen(true);
          }} onRefetch={refetch} />)}
          </div>
        ...
        <TabsContent value="bundles" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {serviceTypes?.filter(s => s.service_type === 'bundle').map(service => <ServiceTypeCard key={service.id} service={service} onEdit={() => {
            setEditingService(service);
            setIsDialogOpen(true);
          }} onRefetch={refetch} />)}
          </div>
        ...
        <TabsContent value="sub" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {serviceTypes?.filter(s => s.service_type === 'sub_service').map(service => <ServiceTypeCard key={service.id} service={service} onEdit={() => {
            setEditingService(service);
            setIsDialogOpen(true);
          }} onRefetch={refetch} />)}
          </div>
      ...
      <ServiceTypeDialog open={isDialogOpen} onOpenChange={open => {
      setIsDialogOpen(open);
      if (!open) setEditingService(null);
    }} serviceType={editingService} onSuccess={() => {
